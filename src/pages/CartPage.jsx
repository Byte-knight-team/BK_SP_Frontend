  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 p-5 px-6 flex flex-col gap-4 max-md:p-4">
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-[60px] px-5 gap-4 rounded-[16px] border border-dashed border-slate-300 bg-white/70">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 text-slate-500">
              <ShoppingCart size={28} />
            </div>
            <p className="text-base font-semibold text-navy">Your cart is empty</p>
            <p className="text-center text-sm text-gray-500">Looks like you haven't added anything yet.</p>
            <button className="px-7 py-2.5 rounded-sm bg-orange text-white text-[0.9rem] font-semibold transition-colors duration-300 hover:bg-orange-hover" onClick={() => navigate('/menu')}>
              Browse Menu
            </button>
          </div>
        ) : (
          cartItems.map((item) => (
            <div className="flex items-center gap-4 p-[18px] px-5 bg-white border border-gray-200 rounded-[14px] transition-shadow duration-300 hover:shadow-card-hover max-[480px]:flex-wrap" key={item.id}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2">
                  <button
                    className="flex h-8 w-8 items-center justify-center rounded-full border border-navy/40 bg-white text-navy transition-all duration-300 hover:border-orange-500 hover:bg-orange-500 hover:text-white"
                  ...
                  <button
                    className="flex items-center justify-center w-8 h-8 rounded-full bg-transparent text-orange"
                    onClick={() => removeFromCart(item.id)}
                  >
                    <Trash2 className="transition-all duration-300 transtion-all hover:border-orange-500  hover:text-red-700" size={18} />
                  </button>
                </div>
              </div>
