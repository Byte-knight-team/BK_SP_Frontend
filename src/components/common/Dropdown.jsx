...
// Reusable dropdown (Headless UI Listbox) — options: [{ value, label }]
// Shared across roles; keep it data-agnostic (no role-specific logic here).
const Dropdown = ({ value, onChange, options, placeholder = 'Select', disabled = false, compact = false, bordered = false }) => {
  ...
  return (
    <Listbox value={value} onChange={onChange} disabled={disabled}>
      <div className="relative">
        <ListboxButton
          className={`flex w-full cursor-pointer items-center justify-between rounded-2xl text-left font-medium outline-none transition-all focus:ring-2 focus:ring-orange-500/20 disabled:opacity-60 ${
        ...
        >
          {options.length === 0 ? (
            ...
            options.map((o) => (
              ...
              >
                <span className="truncate font-medium text-gray-700 group-data-[selected]:font-bold group-data-[selected]:text-orange-600">
                  {o.label}
                </span>
